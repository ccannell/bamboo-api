 ...
 * @returns {Bamboo}
 */
module.exports = function(host, username, password, oauth) {
    "use strict";
     ...
     * @constructor
     */
    var Bamboo = function(host, username, password, oauth) {
    ...
    Bamboo.prototype.getLatestSuccessfulBuildNumber = function(planKey, params, callback) {
        ...
        request({ uri: planUri, oauth: this.oauth }, function(error, response, body) {
     ...
     * @param {Error|null} error - will return null if no error happen
     * @param {String|null} result - if no error will return build details
     */
    /**
     * Returns the list of all builds for a particular build plan key
     *
     * @param {String|Boolean} params - Query string. E.g. "?start-index=25". Could be false
     * @param {getAllBuildsCallback} callback
     * @param {Array=} currentPlans - List of plans available (each plan has a 'key' and a 'name' value)
     */
    Bamboo.prototype.getAllBuilds = function(buildPlanKey, params, callback, currentBuilds) {
        var self = this,
            planUri = self.host + "/rest/api/latest/result/" + buildPlanKey + ".json" + (params || "");
        ...
        request({ uri: planUri, oauth: this.oauth }, function(error, response, body) {
            ...
            currentBuilds.push.apply(currentBuilds, results.result);
  ...
  /**
    ...
    return new Bamboo(host, username, password, oauth);
};
